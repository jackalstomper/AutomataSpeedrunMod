...
void ModChecker::modifyChipInventory() {
  ...
  if (tauntCount < 2) {
    ...
    ChipSlot newTauntChip{228u, 3228u, 25u, 2u, 6u, ~0u, ~0u, ~0u, ~0u, ~0u, ~0u, 0u};
...
ModChecker::ModChecker(uint64_t processRamStart, ModConfig &&modConfig)
    : m_modConfig(std::move(modConfig)), m_inventoryManager(processRamStart + modConfig.getAddresses().itemTableStart),
...
void ModChecker::checkStuff(CComPtr<DxWrappers::DXGIFactoryWrapper> factoryWrapper) {
  if (*m_isWorldLoaded == 1 && *m_playerNameSet == 1) {
    ...
    } else if (!m_tauntChipsAdded && (m_unitDataFlags[7] & 2) &&
               strncmp(m_currentPhase, "52_AB_Danchi_Haikyo", 19) == 0) {
